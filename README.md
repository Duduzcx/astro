...
É um canvas fixo atrás do documento inteiro contando uma história: a humanidade saindo da Terra rumo à tecnologia. O hero é a Terra — um horizonte curvo no pé da tela (o mesmo `planet.ts` com `kind: 'earth'`: oceano, continentes, nuvens, calotas, luzes de cidade na noite, atmosfera), satélites em órbita (`satellites.ts`) e o foguete na superfície. A Terra fica longe da câmera (z −9,9) com tamanho e posição aparentes convertidos por perspectiva, porque perto uma esfera desse tamanho atravessaria a lente; e deitada 90°, porque de pé o horizonte mostrava a calota polar. Na primeira tela de rolagem o foguete decola (`takeoffSpan`, `rocketWindow`) e a Terra recua e some. O foguete é um modelo real: o "SpaceX Starship Block 3" de Clarence365 (Sketchfab, CC BY 4.0), em `public/models/starship.glb`, carregado por `GLTFLoader` com Draco (decoder em `public/draco/`, o mesmo que vem no three) e acoplado em `rocket.ts` pelo `ROCKET_MODEL_URL`: medido, girado (as aletas no eixo x, para abrirem a silhueta; o lado de aço para a câmera), escalado para `height` e assentado com a base em -0,5h, então o bocal, a chama, o brilho, a fumaça e o rastro ficam onde ficariam no torneado. Os materiais entram com `toneMapped = false` e `envMapIntensity` 0,55 (sem tone mapping no composer, o aço refletindo o ambiente a 1,0 vira neve no bloom) e o modelo só entra na cena depois de `renderer.compileAsync` (o `warm` do `createRocket`), para o primeiro frame dele não ser um tranco; aparece num fade de meio segundo. O .glb foi otimizado com gltf-transform a partir do original de 870k vértices: logo e número de série removidos (nada de marca de terceiro), motores decimados (eram metade dos vértices e ocupam vinte pixels), casco decimado de leve, a árvore achatada em dois grupos, `ship` e `booster`, com as malhas fundidas por material (211 malhas viraram 44 chamadas), Draco; 1,6 MB, 306k vértices. As duas instâncias (lançamento e cruzeiro) compartilham a geometria, cada uma clona só os materiais, e em cruzeiro o grupo `booster` some e voa só a nave. Com `ROCKET_MODEL_URL` vazio (ou `model: false`) vale o torneado procedural que existia antes: um lançador de três núcleos com interestágio preto, coifa, propulsores laterais, pernas, grid fins e vinte e sete sinos instanciados, cascos em canvas com painéis, costuras e fuligem. Antes disso foram testados um foguete torneado de tinta acetinada com nove diâmetros e um veículo de aço escovado com telhas pretas; os modelos livres da NASA (Saturn V com bandeiras dos EUA, Ares e Atlas cinza sem textura, SLS só em 7z de 74 MB) não serviam para a marca. Os planetas usam fotografia (`public/space/`, Solar System Scope, CC BY 4.0: Terra de dia e de noite, nuvens, Júpiter, Marte, Lua com tinta azul virando lua de gelo, anel de Saturno) por cima do procedural, que segue valendo enquanto a textura carrega; escurecimento de borda, terminador quente, especular na água, luzes de cidade fotografadas, nuvens numa esfera própria girando em outro ritmo, e o anel do gigante recebe a sombra do planeta. As nuvens da Terra têm volume: o gradiente da própria cobertura na direção da luz diz qual lado da massa está virado para o sol, e a encosta iluminada fica branca enquanto a base fica cinza-azulada; no terminador elas douram, como no pôr do sol visto de cima. A fotografia da superfície passa por uma curva de contraste suave (o dado da NASA é plano de propósito, e sobre fundo escuro lê como lavado), a água tem o glint apertado do sol e a atmosfera ganha espalhamento para a frente, que é o que faz o arco azul do lado do sol ser mais vivo que o da sombra. Os mundos fotografados rochosos e de gelo ganham relevo tirado da própria foto: um Sobel na luminância vira gradiente, o gradiente vira normal no quadro tangente da esfera (com correção da compressão de longitude perto dos polos) e uma oclusão de vale escurece onde o gradiente é forte — é o que tira Marte do ar de bola pintada. A Terra tem ainda mapa normal (relevo fotografado no quadro tangente analítico da esfera; o mapa da Solar System Scope tem o verde apontando para o sul, daí o sinal invertido em y), máscara de água fotografada para o especular, sombra das nuvens no chão (de cada ponto da superfície sobe na direção da luz até uma casca de nuvens mais alta que a real e amostra a foto das nuvens com `textureLod`, sem risco na costura de longitude) e névoa da atmosfera sobre a superfície perto da borda. A casca da atmosfera é fina por tipo (`KIND_SHELL`: 5,5% do raio na Terra, 7% no alvo): o shader mede, na casca `BackSide`, o cosseno entre normal e visada, que vai de zero na borda externa até um valor fixo na borda do planeta, e pinta só essa faixa, clara colada ao limbo e sumindo para fora; atrás do planeta a profundidade da superfície oclui. A redoma larga de antes (16% do raio) foi o que o cliente pediu para diminuir. Relevo por derivadas de tela só no procedural: a derivada de uma amostra bilinear é constante por texel e vira mosaico; chama em duas camadas, fumaça de 200 pontos na CPU, tremor de câmera. O foguete se move de verdade: curva de gravidade (sobe reto e vai tombando para o centro da tela com o eixo no rumo, `arcAt`/`tiltAt` em `TriScene.tsx`), deriva e balanço no tempo, bocal com gimbal, giro lento até na plataforma e vapor de respiro escapando do casco antes da ignição. A inclinação mora num grupo pai e o giro no filho: no mesmo objeto a ordem de Euler faz a inclinação girar junto com o giro, e o bico aponta para fora do rumo metade do tempo. O rastro (`trail.ts`) é uma fita analítica: em vez de guardar posições por frame (que enrolam quando o scroll volta), amostra a própria trajetória um trecho atrás do bocal, quente e estreita na cabeça, larga e rala na cauda, com ruído correndo para trás; no cruzeiro a cauda quase não abre, porque é vácuo. Pelos Serviços e Integrações passa o desfile: Saturno com o anel, Marte, Júpiter enorme e longe, e a Lua com tinta azul virando lua de gelo (`planet.ts` com `kind` gas/rock/ice), cada um na sua profundidade e janela de progresso, entrando por cima e saindo por baixo, com a nave em cruzeiro (`createRocket({ cruise: true })`) varrendo a tela de um lado ao outro entre eles, subindo, chegando perto e se afastando (escala pela fase da curva), com o eixo no rumo e o rastro atrás. O alvo é o Sol (`sun.ts`): chega no Manifesto, fica inteiro pela faixa de vídeo e vira supernova na Missão ("Seu negócio em novas órbitas"), à vista; o buraco negro nasce nos Resultados e fica pelos Projetos. É uma estrela procedural dentro da casca de triângulos: fotosfera com granulação animada (células claras e veias escuras num domínio advectado por outro ruído, como convecção), manchas com umbra, penumbra fibrosa e fáculas em duas faixas de latitude, escurecimento de limbo real e rampa do branco-amarelo ao laranja; cromosfera fina `BackSide` com espículas; coroa num disco aditivo com serpentinas radiais animadas; e proeminências em fitas de arco ancoradas no limbo, subindo e caindo no tempo. No celular a granulação usa menos oitavas e há duas proeminências em vez de quatro. A superfície grava profundidade e desenha antes dos triângulos, então a metade de trás da casca some e sobra a crosta da frente. A explosão não tem janela própria: `planetBreak(mix, form)` lê o estado do campo e vira o relógio `break` de dois módulos. Até `SUN_BURST` a estrela agoniza (granulação acelera, brilho pulsa, rachaduras brancas abrem, a coroa infla); no estouro a superfície clareia e voa em pedaços, e `explosion.ts` assume: clarão do núcleo (forte e curto) com halo, onda de choque em casca e em anel inclinado no plano dos triângulos, ejeta de plasma aditiva (grumos com centro, velocidade e giro em atributo, o voo inteiro no vertex shader) esfriando do branco à escória, centenas ou milhares de brasas em `Points` com arrasto, poeira em quadrados de frente para a câmera, e o remanescente: um disco aditivo de gás ruidoso (nuvens e filamentos, vermelho no meio e azulado na borda, um caroço frio no centro) que fica e só apaga com a opacidade da tabela de scroll. Tudo é função de `break`, então rolar para trás desfaz a supernova sem costura. Dali o mesmo conjunto de triângulos passa pelos astros:
...
Texturas dos astros e do céu em `public/space/`: Solar System Scope (CC BY 4.0, https://www.solarsystemscope.com/textures/), re-encodadas com `sharp-cli` em três degraus (1k, 2k, 4k) a partir das fontes em 8k (Terra dia/noite/nuvens, Júpiter, Saturno, Marte, Lua, Netuno, anel, Via Láctea). A cena carrega o 1k primeiro e substitui pelo pesado quando chega: 4k no desktop (~7,6 MB no total), 2k no celular (uma textura 4k com mipmaps são 32 MB de GPU). A Terra tem também mapa normal (1k/2k/4k) e máscara especular (1k/2k) da mesma fonte, convertidos do TIFF de 8k, e um terceiro degrau de 8k no mapa de dia, em Júpiter e em Saturno (`ultra`, só desktop com `deviceMemory` ≥ 8: são ~180 MB de GPU por textura com mipmaps). 8k é o teto das fontes livres; 10k ou 20k não existem em licença aberta e passariam do limite de textura de muita GPU. No celular tudo vai a 2k, subido para a GPU na chegada. O crédito está no rodapé. O céu (`space.ts`) roda em intensidade 0,75 sobre o onyx da página e, no desktop, soma nebulosas procedurais (dois véus de ruído simplex em cobalto e violeta, mais fortes longe da faixa da galáxia, girando com o céu). Uma versão mais escura do céu foi testada e recusada: o fundo é parte da identidade. Quem escurece por baixo do texto é a `.scene-scrim` (App.tsx): um véu fixo entre a cena e o conteúdo, uniforme no celular e em gradiente no desktop (mais denso à esquerda, onde o texto mora), que não toca o canvas. **Medir celular de verdade.** `scripts/mobile-perf.mjs` roda um iPhone 13 emulado (390×844, razão de pixels 3) com o processador quatro vezes mais lento. Uma janela de 400px no desktop mente: lá a razão de pixels é 1 e a CPU é a do computador, e era com ela que os números bons de antes tinham sido medidos. No aparelho emulado a página roda a ~43 ms por quadro, e o perfil de CPU mostrou onde: metade em trabalho nativo do navegador (composição) e 12% no bundle principal (framer-motion, Lenis), contra menos de 4% na cena 3D inteira. Por isso a conta fechou do lado do DOM: `TriangleDrift` não roda mais abaixo de 1024px (cada instância é um canvas 2D com o próprio laço) e os `Reveal` não animam `filter: blur` no celular (animar filtro repinta o bloco a cada frame). Esse tempo virou qualidade: bloom, relevo da Terra, mais estrelas e mais segmentos nas esferas custaram cerca de um milissegundo.

**Celular.** A cena desenha todo frame também no celular: a 30fps ela andava na metade da cadência do scroll nativo e o deslize parecia arrastado. O custo por frame já pagava a troca (1,5% de frames longos, 17 ms de média a 60fps). O clarão do hero desceu para a altura do horizonte, porque em tela estreita a coluna de texto ocupa a largura toda e ele ficava em cima do título. O buraco negro foi levado para onde a tela do celular está aberta (a chamada e o começo do Processo) e cresce até lá: em Resultados e Projetos os cards ocupam a largura inteira e o disco ficava escondido atrás deles. A explosão do Sol ganhou um trecho mais largo de página, porque num trecho curto ela passava em dois ou três frames de scroll e lia como corte.

**O foguete que some.** O `.glb` pode não chegar: rede ruim, decodificador Draco bloqueado, aparelho lento. Um hero sem o seu assunto é pior do que um foguete desenhado à mão, então o torneado procedural virou função (`buildProcedural`) e entra se o modelo falhar ou demorar mais de oito segundos. Chegando depois disso, o modelo não substitui mais nada — dois foguetes seria pior.

**O piscar.** A tela piscando era o céu: a esfera do panorama girava também no tempo, e com estrelas de um pixel cada ponto andava uma fração de pixel por frame e acendia e apagava — mil pontos fazendo isso ao mesmo tempo lê como tela quebrada. O céu agora gira só com a página (a nebulosa deslizando é o que dá vida ao quadro parado), as estrelas têm piso de 2,5 px com borda macia, cintilam mais devagar, e o limiar que apaga o campo de triângulos ganhou histerese. Diagnóstico por diferença de pixels entre quadros consecutivos com a página parada, não a olho.

**Carga.** O que atrasava a cena não era download, era thread principal — e a peça mais cara era a última suspeita: o prefiltro do mapa de ambiente (`PMREMGenerator.fromScene(RoomEnvironment)`) custava dois segundos mesmo a 32 de resolução, porque o caro é compilar os shaders do próprio prefiltro, não gerar os texels. Ele agora é o primeiro item da fila de aquecimento, depois da primeira pintura; o foguete, único objeto que depende do reflexo, chega bem depois. Com isso o pedido das primeiras texturas caiu de 4,4 s para 2,1 s e o maior bloco de thread principal, de 9,2 s (versão publicada antes) para 2,9 s. As outras causas: o prefiltro do mapa de ambiente a 256 (2,2 s), 27 MB de vídeo que o `autoplay` baixava mesmo com `preload="none"`, todas as texturas subindo para a GPU no instante em que chegavam, a nebulosa assada num desenho só, e a compilação de shader de tudo antes da primeira pintura — em ANGLE/D3D um shader com cinco oitavas de ruído leva segundos. Agora: ambiente a 32, vídeos só a uma tela de distância (`useAutoPauseVideo` guarda o endereço em `data-src`), uma textura por frame subindo pela fila do `warm`, nebulosa em oito tiras, bloom criado depois do quinto frame, astros compilando um por vez em tempo ocioso, e o gerador procedural do `planet.ts` removido do código-fonte por `#define PHOTO_ONLY` quando há fotografia (era metade do tempo da primeira compilação). Medido nesta máquina, o maior bloco de thread principal caiu de 9,2 s para 3,6 s e a cena aparece no primeiro frame. 4k só na Terra do hero; céu e desfile em 2k, e 8k foi removido: subir uma textura dessas trava quase dois segundos e não há um texel a mais para ver. Todas as texturas são WebP (a pasta caiu de 11 MB para 2,8 MB) e o modelo do foguete foi simplificado a um quarto dos vértices (1,6 MB para 1,26 MB) — a 200px de altura na tela ninguém conta polígono.

A supernova do fim fica dentro do Contato ("A sua operação tem a resposta"): explode em 0.946 e apaga até 0.974 no desktop (0.935 a 0.958 no celular), antes do rodapé. Carga: o HTML pré-carrega os degraus de 1k da Terra, das nuvens e da Via Láctea, o `.glb` do foguete e o decodificador Draco; o chunk da cena começa a baixar junto com o bundle (o `lazy` só protege a primeira pintura); o degrau de 8k espera seis segundos para não disputar banda com o que aparece primeiro. Meteoros (`meteors.ts`: uma fila de pontos atrás de uma cabeça com ease-out, um risco a cada 5 a 14 segundos) e as nebulosas mais fortes são os detalhes que impedem o fundo de virar um preto chapado por baixo do véu. No desktop o pós ainda desfoca os cantos (oito amostras num anel que cresce com a distância do centro e com a velocidade do scroll), e os `Reveal` chegam desfocados (`filter: blur`) só em tela larga, porque animar filtro repinta o bloco a cada frame. No celular a cena desenha a 30fps sempre, com menos triângulos e fumaça, e sem mapa normal nem máscara de água na Terra: metade do trabalho por frame no thread principal, para o scroll nativo por cima ficar liso. A razão de pixels do celular é 1,5 com MSAA: a 1,0 a cena era esticada quase três vezes e virava desenho borrado, e o que travava o arrasto não era o preenchimento, eram dois trancos pontuais: cada astro compilava o próprio shader no primeiro frame em que aparecia (100 a 300ms) e cada textura subia para a GPU no primeiro frame em que era usada. Agora todos os programas compilam na carga (`renderer.compileAsync`, com tudo visível por um instante só para a coleta) e cada textura sobe assim que chega (`renderer.initTexture`, o `warm` dos módulos), fora do caminho do scroll. Medido no desktop simulando 400px: 1,4 a 1,5% de frames longos no primeiro 40% da página em regime, com um pico isolado de ~150ms que é o vídeo da faixa entrando (só a partir de md).

| Arquivo | Onde |
| --- | --- |
| `models/starship.glb` | `rocket.ts` — SpaceX Starship Block 3, Clarence365, CC BY 4.0 (https://sketchfab.com/3d-models/spacex-starship-block-3-6f6c6f88a3eb4b4d822fdca66733fbb2); otimizado, sem logo; crédito no rodapé |
| `draco/` | decoder Draco do three (`examples/jsm/libs/draco/gltf`), carregado só quando o modelo carrega |
