...
**Movimento entre seções.** `ui/EnergyDivider` é um feixe de plasma em Canvas 2D: três senoides somadas dão a ondulação (nunca repete), o feixe é desenhado em três passadas (halo, corpo, núcleo), pulsos de dados correm por ele com a cabeça mais clara que a cauda, e raios nascem num ponto do feixe por deslocamento de ponto médio, com galhos, vivem 160–300ms e somem — a cada raio o feixe dá um flash que decai. `intensity="storm"` estala três vezes mais e `hue` (`cobalt` \| `ice` \| `violet`) muda a temperatura de cada costura. Só anima enquanto está na tela (IntersectionObserver com meia tela de folga), para na aba escondida, DPR limitado a 1 no celular e 1,5 no desktop, e vira um frame estático com `prefers-reduced-motion`. Canvas em vez de SVG porque raios gerados a cada frame com três passadas de brilho não cabem no DOM; em vez de WebGL para não abrir um segundo contexto disputando GPU com a cena de fundo.
