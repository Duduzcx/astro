...
**Movimento entre seções.** `ui/WaveBridge` é uma onda senoidal na costura: o caminho tem o dobro da largura e desliza um período em loop (a emenda cai sempre no mesmo desenho), e por cima corre uma "cobra" — um traço curto de `stroke-dasharray` puxado pelo `stroke-dashoffset`. `flip` espelha para as costuras alternarem. `ui/SectionBridge` é uma linha de luz que se desenha do centro para as pontas quando a costura entre duas seções entra na tela, com um clarão por baixo e uma faísca que percorre a linha uma vez — só transform e opacidade. Os cards entram com escala e um brilho que atravessa o vidro uma vez (`.is-entered .graphite-card::after`, disparado pelo `Reveal`); no desktop o mesmo brilho responde ao hover. O trilho do Processo passa por uma mola (`useSpring`) para não repassar aos cards os saltos do scroll por toque.

**O palco tem altura fixa e só cresce.** A barra de URL do celular recolhe e volta o tempo todo durante o scroll. Qualquer coisa amarrada a `innerHeight` — projeção da câmera, buffer do canvas, `maxScroll` — muda junto, e o objeto salta de tamanho de um frame para o outro. Então o canvas é dimensionado pela MAIOR altura já vista (`stageHeight`), fica `fixed top-0` com essa altura em pixels, e `maxScroll` é medido contra ela. Quando a barra some, a tela apenas revela mais de um canvas que já estava desenhado ali: nada reprojeta, nada salta, e não sobra faixa sem desenho. Só uma mudança de **largura** (girar o aparelho, redimensionar a janela) refaz a cena.
